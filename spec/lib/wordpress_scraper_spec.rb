...
class WordpressScraperTest < Test::Unit::TestCase
  ...
  def test_spider_with_wordpress_2_0_content
    ...
    response.expects(:body).returns(File.read(File.join(RAILS_ROOT, 'spec', 'fixtures', 'wordpress_2.0_example.html')))
  ...
  def test_spider_with_wordpress_2_2_content
    response = Net::HTTPSuccess.new(nil, nil, nil)
    response.expects(:body).returns(File.read(File.join(RAILS_ROOT, 'spec', 'fixtures', 'wordpress_2.2_example.html')))
    Net::HTTP.expects(:get_response).returns(response)
