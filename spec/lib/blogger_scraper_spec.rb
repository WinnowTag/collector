...
class BloggerScraperTest < Test::Unit::TestCase
  ...
  def test_scrape_gets_post_content
    mock_response = mock('response', :body => File.read(File.join(RAILS_ROOT, 'spec', 'fixtures', 'blogger_example.html')))
  ...
  def test_spider_with_blogger_content
    ...
    response.expects(:body).returns(File.read(File.join(RAILS_ROOT, 'spec', 'fixtures', 'blogger_example.html')))
