...
class FeedTest < Test::Unit::TestCase
  ...
  def test_title_with_mixed_content
    test_feed_url = 'file:/' + File.join(File.expand_path(RAILS_ROOT), 'test', 'fixtures', 'feed_with_mixed_content.xml')
  ...
  def test_feed_with_multiple_root_elements
    test_feed_url = 'file:/' + File.join(File.expand_path(RAILS_ROOT), 'test', 'fixtures', 'feed_with_multiple_root_elements.xml')
    feed = FeedTools::Feed.open(test_feed_url)
    assert_equal "Home of Best Gay Blogs", feed.title
  end
  
  def test_feed_with_invalid_html_inside_cdata
    test_feed_url = 'file:/' + File.join(File.expand_path(RAILS_ROOT), 'test', 'fixtures', 'feed_with_invalid_cdata.xml')
    feed = FeedTools::Feed.open(test_feed_url)    
    assert_equal 'Ryan Arrowsmith', feed.title
  end
  
  def test_feed_with_non_utf8_encoding
    test_feed_url = 'file:/' + File.join(File.expand_path(RAILS_ROOT), 'test', 'fixtures', 'non_utf8_feed.rss')
  ...
  def test_feed_with_non_utf8_encoding_via_http
    ...
    response.expects(:body).returns(File.read(File.join(File.expand_path(RAILS_ROOT), 'test', 'fixtures', 'non_utf8_feed.rss')))
